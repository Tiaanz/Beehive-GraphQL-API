...
export const jobResolvers = {
  ...
  Mutation: {
    ...
    applyJob: async (_: any, { jobID, relieverID }, { userRole }) => {
      try {
        if (userRole !== 'RELIEVER') {
          throw ForbiddenError('You are not authorised.')
        }
        ...
        if (
          error.message.includes('relieverId') ||
    ...
    declineJob: async (_: any, { jobID, relieverID }, { userRole }) => {
      try {
        if (userRole !== 'RELIEVER') {
          throw ForbiddenError('You are not authorised.')
        }
    ...
    acceptJob: async (_: any, { jobID, relieverID }, { userRole }) => {
      try {
        if (userRole !== 'MANAGER') {
          throw ForbiddenError('You are not authorised.')
        }
    ...
    getJob: async (_: any, { relieverID, jobID }, { userRole }) => {
      try {
        if (userRole !== 'MANAGER') {
          throw ForbiddenError('You are not authorised.')
        }
